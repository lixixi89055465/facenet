...
for n_iter in range(100):
    ...
    if n_iter % 10 == 0:
        ...
        writer.add_text('Text', 'texdt logged at step:' + str(n_iter), n_iter)
        for name, param in resnet18.named_parameters():
            writer.add_histogram(name, param.clone().cpu().data.numpy(), n_iter)
        # need tensorboard 0.4RC or later
...
# dxport scalar data to JSON for external processing
writer.export_scalars_to_json('./all_scalars.json')
writer.close()
# tensorboard --logdir=./path/to/the/folder --port 8123
# 重新确定torch和 tensorboard 版本
